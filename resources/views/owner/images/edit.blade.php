                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('owner.images.update', ['image' => $image->id]) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="-m-2">
                            ...
                            <div class="p-2 w-full flex justify-center space-x-4 md:space-x-16 mt-4">
                    ...
                    <form id="delete_{{ $image->id }}" action="{{ route('owner.images.destroy', ['image' => $image->id]) }}" method="post">
                        @csrf
                        @method('delete')
                        <div class="p-2 w-full text-center mt-8">
                            <button type="button" data-id="{{ $image->id }}" onclick="deletePost(this)" class="text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded text-lg">削除する</button>
                        </div>
                    </form>
                </div>
            </div>
